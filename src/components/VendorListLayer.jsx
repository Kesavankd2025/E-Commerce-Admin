...
const VendorListLayer = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await VendorApi.getAll(currentPage, rowsPerPage, debouncedSearchTerm);
    ...
    const handleDelete = async () => {
        if (itemToDelete) {
            const response = await VendorApi.delete(itemToDelete._id);
    ...
    return (
        <div className="card h-100 p-0 radius-12">
            <div className="card-header border-bottom bg-base py-16 px-24 d-flex align-items-center justify-content-between flex-wrap gap-3">
                <h6 className="text-primary-600 pb-2 mb-0">Vendor List</h6>
                <div className="d-flex align-items-center flex-wrap gap-3">
                    ...
                    {hasPermission("Vendor", "add") && (
                        <Link to="/vendor/add" className="btn btn-primary text-sm btn-sm px-12 py-12 radius-8 d-flex align-items-center gap-2">
                            <Icon icon="ic:baseline-plus" className="text-xl" /> Add Vendor
                        </Link>
                    )}
                </div>
            </div>
                    ...
                    <table className="table bordered-table sm-table mb-0">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" style={{ color: "black" }}>Vendor Name</th>
                                <th scope="col" style={{ color: "black" }}>Contact Person</th>
                                <th scope="col" style={{ color: "black" }}>Phone</th>
                                <th scope="col" style={{ color: "black" }}>GST No</th>
                            ...
                            {loading ? (
                                ...
                                data.map((item, index) => (
                                    <tr key={item._id || index}>
                                        ...
                                        <td><span className="text-md mb-0 fw-normal text-secondary-light">{item.contactPerson}</span></td>
                                        <td><span className="text-md mb-0 fw-normal text-secondary-light">{item.phoneNumber}</span></td>
                                        <td><span className="text-md mb-0 fw-normal text-secondary-light">{item.gstNumber || "N/A"}</span></td>
                                        <td>
                                            <span className={`badge ${item.status ? "bg-success-focus text-success-600" : "bg-danger-focus text-danger-600"} px-24 py-4 radius-4 fw-medium text-sm`}>
                                        ...
                                        <td>
                                            <div className="d-flex align-items-center gap-2">
                                                {hasPermission("Vendor", "view") && (
                                                    <Link to={`/vendor/view/${item._id}`} className="bg-info-focus bg-hover-info-200 text-info-600 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle">
                                                        <Icon icon="majesticons:eye-line" className="menu-icon" />
                                                    </Link>
                                                )}
                                                {hasPermission("Vendor", "edit") && (
                                                    <Link to={`/vendor/edit/${item._id}`} className="btn-edit-custom fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle">
            ...
            <Modal show={showDeleteModal} onHide={handleCloseDeleteModal} centered>
                <Modal.Body className="text-center p-5">
                    ...
                    <p className="text-secondary-light mb-4">Do you want to delete this Vendor?</p>
