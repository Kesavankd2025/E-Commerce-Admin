...
const UnitFormLayer = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    let result;
    if (id) result = await UnitApi.update(id, payload);
    else result = await UnitApi.create(payload);
    setLoading(false);
    if (result.status) navigate("/units");
  };

  return (
    <div className="card h-100 p-0 radius-12">
      <div className="card-header border-bottom bg-base py-16 px-24">
        <h6 className="text-primary-600 pb-2 mb-0">{isView ? "View" : id ? "Edit" : "Create"} Units</h6>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="row gy-4">
            <div className="col-md-6">
              ...
              <input type="text" className="form-control radius-8" name="name" value={formData.name} onChange={handleChange} placeholder="Enter Unit Name" disabled={isView} />
              {errors.name && <small className="text-danger">{errors.name}</small>}
            </div>
            <div className="col-md-6">
              <label className="form-label fw-semibold">Short Name</label>
              <input type="text" className="form-control radius-8" name="shortName" value={formData.shortName} onChange={handleChange} placeholder="Enter Short Name" disabled={isView} />
              {errors.shortName && <small className="text-danger">{errors.shortName}</small>}
            </div>
          ...
          <div className="d-flex justify-content-end gap-2 mt-24">
            <Link to="/units" className="btn btn-outline-danger-600 px-32 radius-8 justify-content-center" style={{ width: "120px" }}>Cancel</Link>
            {!isView && hasPermission("Unit", id ? "edit" : "add") && (
