...
const BrandFormLayer = () => {
  ...
  const fetchDetails = async () => {
    ...
    if (response.status) {
      ...
      if (data.logo) {
        const imagePath = data.logo?.path || data.logo;
  ...
  const handleSubmit = async (e) => {
    ...
    if (imageFile) {
      ...
      const uploadRes = await ImageUploadApi.uploadImage({ formData: form, path: "brands" });
    ...
    payload["logo"] = finalImage;
    payload.isActive = payload.status;
    let result;
    if (id) result = await BrandApi.update(id, payload);
    else result = await BrandApi.create(payload);
    setLoading(false);
    if (result.status) navigate("/brands");
  };

  return (
    <div className="card h-100 p-0 radius-12">
      <div className="card-header border-bottom bg-base py-16 px-24">
        <h6 className="text-primary-600 pb-2 mb-0">{isView ? "View" : id ? "Edit" : "Create"} Brands</h6>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="row gy-4">
            <div className="col-md-6">
              ...
              <input type="text" className="form-control radius-8" name="name" value={formData.name} onChange={handleChange} placeholder="Enter Brand Name" disabled={isView} />
          ...
          <div className="d-flex justify-content-end gap-2 mt-24">
            <Link to="/brands" className="btn btn-outline-danger-600 px-32 radius-8 justify-content-center" style={{ width: "120px" }}>Cancel</Link>
            {!isView && hasPermission("Brands", id ? "edit" : "add") && (
