...
class CategoryApi {
  async getAll(page = 0, limit = 10, search = "") {
    try {
      const url = `/category/list?page=${page}&limit=${limit}&search=${search}`;
  ...
  async getById(id) {
    try {
      const response = await apiClient.get(`/category/details/${id}`);
      if (response.status === 200 || response.status === 201) return { status: true, response: response.data };
    } catch (error) {
  ...
  async create(data) {
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ShowNotifications.showAlertNotification(response.data.message || "Category created successfully!", true);
  ...
  async update(id, data) {
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ShowNotifications.showAlertNotification(response.data.message || "Category updated successfully!", true);
        return { status: true, response: response.data };
      }
    } catch (error) {
  ...
  async delete(id) {
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ShowNotifications.showAlertNotification(response.data.message || "Category deleted successfully!", true);
        return { status: true, response: response.data };
      }
    } catch (error) {
  ...
  async statusUpdate(id) {
    try {
      const response = await apiClient.put(`/category/${id}/toggle-status`);
      if (response.status === 200 || response.status === 201) {
